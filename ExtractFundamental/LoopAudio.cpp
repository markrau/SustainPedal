...
void LoopAudio::getPitchPeriod(int *curInBuf){
	...
	for(int i = 1; i < buf_len; i++){
		...
		prevVal -  thresh < curInBuf[i-1] && curInBuf[i-1] < thresh + prevVal){
			s1 = i+1;
			cout << "Found at position :" << s1 << endl;
			break;
		}
	}
	...
	for(int i = s1; i < buf_len; i++){
		...
		prevVal -  thresh < curInBuf[i-1] && curInBuf[i-1] < thresh + prevVal){
			s2 = i+1;
			cout << "Found again at position :" << s2 << endl;
			break;
		}
	}
...
void LoopAudio::loopBuffer(int *curInBuf, int *curOutBuf){
